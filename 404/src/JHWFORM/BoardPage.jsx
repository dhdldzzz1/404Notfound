...
export default function BoardPage() {
  ...
  /** 카테고리/검색 필터 (데모에선 카테고리 기능은 의도만 남겨둠) */
  const filtered = useMemo(() => {
    const byKeyword = SAMPLE_ROWS.filter(r =>
      r.title.toLowerCase().includes(q.trim().toLowerCase())
      || r.writerId.toLowerCase().includes(q.trim().toLowerCase())
      || r.dept.toLowerCase().includes(q.trim().toLowerCase())
    );
    // 카테고리 로직을 실제 필드로 연결하면 여기에서 분기
    return byKeyword;
  }, [q, category]);
  ...
  return (
    <div className="boardpage">
      ...
      <form className="toolbar" onSubmit={handleSearch}>
        ...
        <button
          type="button"
          className="toolbar__write"
          onClick={() => alert("글쓰기 라우팅 연결해주세요 (예: /write)")}
        >
          글쓰기
        </button>
      </form>
              ...
              pageRows.map((r) => (
                <tr key={r.no}>
                  ...
                  <td className="title-cell">
                    <button
                      className="linklike"
                      onClick={() => alert(`상세보기 라우팅: /view/${r.no}`)}
                    >
                      {r.title}
                    </button>
                  </td>
